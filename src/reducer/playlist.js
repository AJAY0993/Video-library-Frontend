...
export async function getPlaylists(payload, dispatch) {
  const token = localStorage.getItem("token")
  const configuration = {
    method: "GET",
    url: `${BASE_URL}users/${payload.userId}/playlists`,
    headers: {
...
export async function createPlaylist(payload, dispatch) {
  dispatch({ type: "SET_LOADER", payload: true })
  const token = localStorage.getItem("token")
  const configuration = {
    method: "POST",
...
export async function addVideoToPlaylist(payload, dispatch) {
  try {
    const token = localStorage.getItem("token")
    const configuration = {
      method: "PATCH",
...
export async function deletePlaylist({ state, action }, dispatch) {
  ...
  try {
    ...
    dispatch({
      ...
      payload: state.playlists.filter((playlist) => playlist._id !== playlistId)
    })
  } catch (err) {
    console.log(err)
    dispatch({ type: "SET_LOADER", payload: false })
