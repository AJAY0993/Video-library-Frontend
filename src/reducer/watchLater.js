...
export async function getWatchLater(payload, dispatch) {
  const token = localStorage.getItem("token")
  const configuration = {
    method: "GET",
    url: `${BASE_URL}users/${payload.userId}/playlists?playlistName=watchLater`,
  ...
  try {
    ...
    const data = res.data
    console.log(data)
    dispatch({ type: "SET_WATCHLATER", payload: data.data.playlists[0].videos })
